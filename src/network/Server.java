...
public class Server extends AbstractServer {
    ...
    private void handleCreateRoom(String message,ConnectionToClient client){

        String[] temp =  message.split(",");
        String player = temp[1];
        String roomName = temp[2];

        String name = client.getInfo("name").toString();
        int id = Integer.parseInt(client.getInfo("id").toString());
        Room room = new Room(roomName);
        Player playerObj = new Player(name, id);
    ...
    private void handleJoin(String message,ConnectionToClient client){
        ...
        for(Room room: rooms){
            if(room.getRoomNumber() == roomNum && !room.isFull()){
                String name = client.getInfo("name").toString();
                int id = Integer.parseInt(client.getInfo("id").toString());
                Player playerObj = new Player(name, id);
                ...
                sendToAllClients(String.format("#setTurn,%d,%s",player1,"true"));
    ...
    private void handlePlay(String message) {
        ...
        String tempCard = split[2];

        Player playerPlay;
        int player1;
        int player2;
        for (Room room : rooms) {
            if (room.getPlayer1().getID() == player) {
                playerPlay = room.getPlayer1();
                player1 = room.getPlayer1().getID();
                player2 = room.getPlayer2().getID();
            }else return;
            ...
            if (room.play(tempCard, playerPlay)) {
                ...
                ///
                System.out.println("cardto send" + cardToSend);
